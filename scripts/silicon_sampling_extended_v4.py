#!/usr/bin/env python3
"""
Silicon Sampling Validation Framework - VERSION 4 (Multi-Country)
=================================================================
Major changes from v3:
  1. FULL ESS11: All 30 countries (replaces ESS9 GB-only)
  2. DYNAMIC BACKSTORY: Generated from dataframe demographics (no static CSV)
  3. SOCIAL TRUST FOCUS: Breyer Social Trust Scale (ppltrst, pplfair, pplhlp)
  4. COUNTRY-LEVEL ANALYSIS: Survey vs silicon trust scatterplots
  5. VARIABLE DICTIONARY: Updated to 42 variables available in ESS11
  6. OVERTIME FRAMEWORK: Placeholder for multi-round ESS analysis

Usage:
    # Social trust only (recommended first run)
    python silicon_sampling_extended_v4.py --block trust --model qwen

    # Social trust for specific countries
    python silicon_sampling_extended_v4.py --block trust --model qwen --countries GB FR DE

    # Full variable set for selected countries
    python silicon_sampling_extended_v4.py --block full --model qwen --countries GB FR

    # All blocks (trust + full + mechanisms + country scatter)
    python silicon_sampling_extended_v4.py --block all --model qwen --countries GB FR DE

    # Model comparison (RQ8)
    python silicon_sampling_extended_v4.py --block rq8 --model qwen --countries GB FR DE
"""
...
def generate_backstory(row):
    ...
    # Bounds check: age should be 0-120, birth year 1880-2010
...
def generate_backstories_for_df(df):
    """
    ...
    """
    logging.info(f"Generating backstories for {len(df)} respondents...")
    backstories = df.apply(generate_backstory, axis=1)
...
def load_data(ess_file=None, countries=None, sample_per_country=None, seed=888):
    ...
    alt_vars = ['marital', 'maritala', 'hinctnt']
    ...
    df['backstory'] = generate_backstories_for_df(df)
...
def run_social_trust(df, model_name='qwen', seed=888):
    """
    ...
    """
    logger = setup_logging('social_trust', model_name)
    logger.info(f"=== SOCIAL TRUST ANALYSIS (Model: {model_name}) ===")
    ...
    raw_file = os.path.join(RESULTS_DIR, f"silicon_trust_raw_{model_name}_seed{seed}.csv")
    ...
    indiv_file = os.path.join(RESULTS_DIR, f"silicon_trust_individual_{model_name}.csv")
    ...
    scatter_file = os.path.join(RESULTS_DIR, f"silicon_trust_country_scatter_{model_name}.csv")
    ...
    composite_file = os.path.join(RESULTS_DIR, f"silicon_trust_composite_scatter_{model_name}.csv")
...
def run_full_variables(df, model_name='qwen', seed=888):
    """
    ...
    """
    logger = setup_logging('full_vars', model_name)
    logger.info(f"=== FULL VARIABLE SET (Model: {model_name}) ===")
    ...
    results_df = pd.DataFrame(results_data)
    raw_file = os.path.join(RESULTS_DIR, f"silicon_full_raw_{model_name}_seed{seed}.csv")
    results_df.to_csv(raw_file, index=False)
    logger.info(f"Raw data saved to {raw_file}")
    ...
    rq1_df.to_csv(os.path.join(RESULTS_DIR, f"silicon_full_rq1_{model_name}.csv"), index=False)
    ...
    scatter_df = pd.DataFrame(scatter_results)
    scatter_df.to_csv(os.path.join(RESULTS_DIR, f"silicon_full_country_scatter_{model_name}.csv"),
                      index=False)
...
def run_deep_analysis(df, model_name='qwen', seed=888):
    ...
    # Load social trust results
    raw_file = os.path.join(RESULTS_DIR, f"silicon_trust_raw_{model_name}_seed{seed}.csv")
    if not os.path.exists(raw_file):
        logger.error(f"Social trust results not found: {raw_file}. Run 'trust' block first.")
        return None
...
def main():
    parser = argparse.ArgumentParser(
        description='Silicon Sampling v4 — Multi-Country ESS11 Framework',
...
Examples:
  python silicon_sampling_extended_v4.py --block trust --countries GB FR DE
  python silicon_sampling_extended_v4.py --block all --countries GB FR DE NL SE
  python silicon_sampling_extended_v4.py --block full --countries GB
        """)
    ...
    print(f"\n{'=' * 70}")
    print(f"SILICON SAMPLING v4 — Multi-Country ESS11")
    print(f"{'=' * 70}")
