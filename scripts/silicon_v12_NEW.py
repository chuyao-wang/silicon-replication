...
def generate_backstory(row, mode='v5_clean'):
    ...
    include_ses = mode not in ('minimal', 'demo_only', 'minimal_politics')  # v12.2: minimal_politics = minimal + political block
    ...
    _LOGO_MODES = ('full_noascriptive',   # G2: gender, age, birth year
                   'full_nosocioecon',    # G3: education, main activity, income decile
                   'full_nohousehold',    # G4: marital status, household size, children
                   'full_nocivic',        # G5: born in country, citizenship
                   'full_nomarkers',      # G7: union, discrimination, internet use
                   'full_nodomicil')      # G1 remainder: the domicile phrase
    _logo = mode if mode in _LOGO_MODES else None
    include_v5_extras = mode in (('v5', 'v5_clean', 'full_clean', 'full_nocountry', 'full_noregion', 'full_nopolitical') + _LOGO_MODES)  # v10 CHANGE; v12.4 adds the LOGO arms
    include_region = mode not in ('full_nocountry', 'full_noregion')  # v10 CHANGE (B1): both no-* arms drop the NUTS code, so full_noregion vs full_nocountry differ ONLY in the country sentence
    ...
    # Gender
    gndr = _safe_int(row.get('gndr')) if _logo != 'full_noascriptive' else None  # v12.4
    gender_str = GENDER_MAP.get(gndr)
    if gender_str:
        parts.append(f"I am {gender_str}.")

    # Age and birth year
    agea = _safe_int(row.get('agea')) if _logo != 'full_noascriptive' else None  # v12.4
    yrbrn = _safe_int(row.get('yrbrn')) if _logo != 'full_noascriptive' else None  # v12.4
    ...
    if include_ses:
        # Marital status
        marital = _safe_int(row.get('maritalb')) if render_family and _logo != 'full_nohousehold' else None  # v12.4
        ...
        # Region and domicile
        domicil = _safe_int(row.get('domicil')) if render_geo and _logo != 'full_nodomicil' else None  # v12.4
        domicil_str = DOMICIL_MAP.get(domicil)
        region = row.get('region', None)
        if domicil_str and region and not pd.isna(region) and include_region:  # v8 CHANGE
            parts.append(f"I live in {domicil_str} in the {region} region.")
        elif domicil_str:
            parts.append(f"I live in {domicil_str}.")
        elif _logo == 'full_nodomicil' and region and not pd.isna(region) and include_region:
        ...
        brncntr = _safe_int(row.get('brncntr')) if render_geo and _logo != 'full_nocivic' else None  # v12.4
        if brncntr == 1:
            parts.append(f"I was born in this country.")
        elif brncntr == 2:
            parts.append(f"I was not born in this country.")

        ctzcntr = _safe_int(row.get('ctzcntr')) if render_geo and _logo != 'full_nocivic' else None  # v12.4
        if ctzcntr == 1:
        ...
        # Education
        eisced = _safe_int(row.get('eisced')) if _logo != 'full_nosocioecon' else None  # v12.4
        eisced_str = EISCED_MAP.get(eisced)
        eduyrs = _safe_int(row.get('eduyrs')) if _logo != 'full_nosocioecon' else None  # v12.4
        ...
        # Main activity
        mainact = _safe_int(row.get('mainact')) if _logo != 'full_nosocioecon' else None  # v12.4
        ...
        hinctnta = _safe_int(row.get('hinctnta')) if _logo != 'full_nosocioecon' else None  # v12.4
    ...
    if include_v5_extras or mode == 'minimal_politics_econ':  # v12.3
        ...
        mbtru = _safe_int(row.get('mbtru')) if _logo != 'full_nomarkers' else None  # v12.4
        if mbtru == 1:
        ...
        dscrgrp = _safe_int(row.get('dscrgrp')) if include_v5_extras and _logo != 'full_nomarkers' else None  # v12.4
        if dscrgrp == 1:
        ...
        netusoft = _safe_int(row.get('netusoft')) if _logo != 'full_nomarkers' else None  # v12.4
