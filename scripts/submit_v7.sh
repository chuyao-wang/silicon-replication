...
#     5. minimal (4 vars)    6. ses (14 vars)
#     7. political (16 vars) 8. full_clean (20 vars, = main Qwen 1P)
#
# Usage:
#   bash submit_v7.sh                       # all 8 jobs, 400/country
#   bash submit_v7.sh all 1                 # PILOT: 1/country (~30s each)
#   bash submit_v7.sh main 400             # main 4 jobs only
#   bash submit_v7.sh ablation 400         # ablation 4 jobs only
# ============================================================================
...
# --- ABLATION: Qwen 1P only, 4 backstory levels ---
if [ "$MODE" = "all" ] || [ "$MODE" = "ablation" ]; then
    echo "--- Backstory ablation (Qwen 1P) ---"
    for BS in minimal ses political full_clean; do
