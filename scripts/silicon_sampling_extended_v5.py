...
def select_prompt_template(prompt_mode):
    templates = {"1p": PROMPT_1P_STANDARD, "3p": PROMPT_3P, "1p_idiosyncratic": PROMPT_1P_IDIOSYNCRATIC}
    return templates.get(prompt_mode, PROMPT_1P_STANDARD)
...
def generate_backstory(row, mode='v5'):
    """
    ...
    mode : str
        'v4' = original 17 demographic variables only (parsimonious)
        'v5' = expanded 27 variables (includes social, political, household)
    ...
    # --- NEW backstory elements (v5, expanded per supervisor feedback) ---
    # Only included when mode='v5'; omitted in mode='v4' for ablation test
    if mode == 'v5':
...
def generate_backstories_for_df(df, mode='v5'):
    """
...
def run_social_trust(df, model_name="qwen", seed=888, prompt_mode="1p", temperature=0):
    """
    ...
    for var_name, var_info in trust_vars.items():
        for idx, row in df.iterrows():
            ...
            prompt = select_prompt_template(prompt_mode).format(
                backstory=backstory,
...
def run_full_variables(df, model_name="qwen", seed=888, prompt_mode="1p", temperature=0):
    """
...
def main():
    parser = argparse.ArgumentParser(
        description='Silicon Sampling v5 — Multi-Country ESS11 Framework (Expanded Backstory)',
...
Examples:
  ...
  python silicon_sampling_extended_v5.py --block full --countries GB
    ...
    parser.add_argument('--prompt', type=str, default='1p',
                        ...
                        help='Prompt framing mode')
    parser.add_argument('--temperature', type=float, default=0,
                        help='Sampling temperature (default: 0)')
    parser.add_argument('--backstory', type=str, default='v5',
                        choices=['v4', 'v5'],
                        help='Backstory mode: v4 (17 vars) or v5 (27 vars, default)')
    ...
    print(f"\n{'=' * 70}")
    print(f"SILICON SAMPLING v5 — Multi-Country ESS11 (Expanded Backstory)")
    print(f"{'=' * 70}")
    print(f"Block:      {args.block}")
    print(f"Model:      {args.model}")
    print(f"Backstory:  {args.backstory} ({'27 variables' if args.backstory == 'v5' else '17 variables'})")
    ...
    if args.block == 'trust':
        run_social_trust(df, model_name=args.model, seed=args.seed, prompt_mode=args.prompt, temperature=args.temperature)

    elif args.block == 'full':
        run_full_variables(df, model_name=args.model, seed=args.seed, prompt_mode=args.prompt, temperature=args.temperature)
