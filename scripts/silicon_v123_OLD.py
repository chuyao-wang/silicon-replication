...
def generate_backstory(row, mode='v5_clean'):
    ...
    include_v5_extras = mode in ('v5', 'v5_clean', 'full_clean', 'full_nocountry', 'full_noregion', 'full_nopolitical')  # v10 CHANGE
    ...
    if include_ses:
        # Marital status
        marital = _safe_int(row.get('maritalb')) if render_family else None
        ...
        # Region and domicile
        domicil = _safe_int(row.get('domicil')) if render_geo else None
        ...
        brncntr = _safe_int(row.get('brncntr')) if render_geo else None
        if brncntr == 1:
            parts.append(f"I was born in this country.")
        elif brncntr == 2:
            parts.append(f"I was not born in this country.")

        ctzcntr = _safe_int(row.get('ctzcntr')) if render_geo else None
        if ctzcntr == 1:
    ...
    if include_v5_extras or mode == 'minimal_politics_econ':  # v12.3
        ...
        dscrgrp = _safe_int(row.get('dscrgrp')) if include_v5_extras else None
        if dscrgrp == 1:
...
Examples:
  ...
  python silicon_sampling_extended_v5.py --block full --countries GB
    ...
    parser.add_argument('--backstory', type=str, default='v5_clean',
                        choices=['v4', 'v5', 'v5_clean', 'demo_only', 'minimal', 'ses', 'political', 'full_clean', 'full_nocountry', 'full_noregion', 'full_nopolitical', 'minimal_politics', 'minimal_politics_ses', 'minimal_politics_ses_geo', 'minimal_politics_econ'],  # v10 CHANGE; v12.2-3 add the regrouped rungs
